...
const buildCoTrendData = (
  ...
  const pollutionScenarioRatio = (
    scenario.coChangePct * 0.7 +
    scenario.pm25ChangePct * 0.2 +
    scenario.no2ChangePct * 0.1
  ) / 100;
  const ecologicalScenarioRatio = (
    scenario.deforestationPressurePct * 0.35 -
    scenario.biodiversityResiliencePct * 0.22 -
    scenario.waterQualityResiliencePct * 0.18 +
    ecologicalShiftPct * 0.25
  ) / 100;
  const scenarioImpactRatio = pollutionScenarioRatio + ecologicalScenarioRatio;
  ...
  const coVolatilityRatio = clamp(stdDev(safePoints.map((point) => point.y)) / Math.max(1, fallback), 0, 1.2);
  const recentPoints = safePoints.slice(-3);
  const recentSlope = recentPoints.length >= 2
    ? (recentPoints[recentPoints.length - 1].y - recentPoints[0].y)
      / Math.max(1, recentPoints[recentPoints.length - 1].x - recentPoints[0].x)
    : 0;
  const longSlope = safePoints.length >= 2
    ? (safePoints[safePoints.length - 1].y - safePoints[0].y)
      / Math.max(1, safePoints[safePoints.length - 1].x - safePoints[0].x)
    : 0;
  const slopeMomentum = recentSlope - longSlope;
  const latestNo2 = yearlyMetrics.length ? yearlyMetrics[yearlyMetrics.length - 1].no2 : 0;
  const urbanPressureRatio = clamp(latestNo2 / 120, 0, 0.9);
  const heatwavePressure = environmentContext ? clamp(environmentContext.heatwaveDaysNextWeek / 10, 0, 1) : 0;
  const humidityPressure = environmentContext ? clamp(environmentContext.humidity / 100, 0, 1) : 0;
  const windRelief = environmentContext ? clamp(environmentContext.windSpeed / 28, 0, 1) : 0;
  const climatePressure = (heatwavePressure * 0.35) + (humidityPressure * 0.2) - (windRelief * 0.22);
  const geoModifier = city
    ? clamp((Math.abs(city.lat - 23) / 35) + (Math.abs(city.lon - 80) / 60), 0.65, 1.35)
    : 1;
  const cityTrendFactor = 1 + (coVolatilityRatio * 0.25) + (urbanPressureRatio * 0.2) + (climatePressure * 0.18);
  ...
  const forecastValues = futureYears.map((year) => {
    ...
    const horizonFactor = 1 + ramp * 0.25;
    const momentumBend = slopeMomentum * yearsFromNow * yearsFromNow * 0.08 * geoModifier;
    const cityAdjustedBase = Math.max(0, baseValue + momentumBend);
    const scenarioAdjusted = cityAdjustedBase * liveCalibration * (1 + scenarioImpactRatio * ramp * horizonFactor * cityTrendFactor);
    return Math.max(0, scenarioAdjusted);
  });

  const forecastLowerValues = forecastValues.map((value, index) => {
    const growthFactor = 1 + index * (0.11 + coVolatilityRatio * 0.1 + Math.abs(climatePressure) * 0.05);
    const spread = Math.max(0.2, residual * (1 + urbanPressureRatio * 0.45) * geoModifier * growthFactor);
...
const buildScenarioAqiData = (
  ...
  const pollutionImpactRatio = (
    scenario.pm25ChangePct * 0.5 +
    scenario.no2ChangePct * 0.35 +
    scenario.coChangePct * 0.15
  ) / 100;
  const ecologyImpactRatio = (
    scenario.deforestationPressurePct * 0.4 -
    scenario.biodiversityResiliencePct * 0.3 -
    scenario.waterQualityResiliencePct * 0.3 +
    ecologicalShiftPct * 0.3
  ) / 100;
  const monthAnomalyProfile = avgBase > 0
    ? baseAqiData.values.map((value) => clamp(value / avgBase, 0.72, 1.35))
    : baseAqiData.values.map(() => 1);
  const phaseShift = city ? ((city.lat * 0.11 + city.lon * 0.07) % 1) * Math.PI * 2 : 0;
  const humidityFactor = environmentContext ? clamp(environmentContext.humidity / 100, 0, 1) : 0.5;
  const precipitationFactor = environmentContext ? clamp(environmentContext.precipitation / 12, 0, 1) : 0.2;
  const heatwaveFactor = environmentContext ? clamp(environmentContext.heatwaveDaysNextWeek / 10, 0, 1) : 0;
  const windFactor = environmentContext ? clamp(environmentContext.windSpeed / 30, 0, 1) : 0.3;

  const values = baseAqiData.values.map((value, index) => {
    const seasonalWave = Math.sin((index / 12) * Math.PI * 2 + phaseShift);
    const seasonalWeight = 0.9 + (seasonalWave + 1) * 0.14;
    const monsoonWeight = index >= 5 && index <= 8 ? 1 : 0;
    const preSummerWeight = index >= 2 && index <= 4 ? 1 : 0;
    const winterWeight = index === 11 || index <= 1 ? 1 : 0;
    const climateShape = 1
      + preSummerWeight * (heatwaveFactor * 0.2)
      - monsoonWeight * ((humidityFactor * 0.13) + (precipitationFactor * 0.1))
      + winterWeight * (windFactor * 0.06);
    const monthShape = clamp((monthAnomalyProfile[index] ?? 1) * seasonalWeight * climateShape, 0.68, 1.45);
    const adjusted = value * liveCalibration * (1 + (pollutionImpactRatio * monthShape) + (ecologyImpactRatio * climateShape));
...
const Predictions = () => {
    ...
    const loadCityData = async () => {
      ...
      try {
        ...
        const [live, historical, environment] = await Promise.all([
          ...
          fetchEnvironmentalContext(selectedCity.lat, selectedCity.lon, {
            preferLive: true,
            allowStaleCache: true,
          }),
        ]);
  ...
  const co2 = useMemo(() => {
    if (!yearlyMetrics.length) return null;
    return buildCoTrendData(yearlyMetrics, year, scenario, currentCo, ecologicalShiftPct, selectedCity, environmentContext);
  }, [yearlyMetrics, year, scenario, currentCo, ecologicalShiftPct, selectedCity, environmentContext]);

  const baseCo2 = useMemo(() => {
    if (!yearlyMetrics.length) return null;
    return buildCoTrendData(yearlyMetrics, year, ZERO_SCENARIO, currentCo, 0, selectedCity, environmentContext);
  }, [yearlyMetrics, year, currentCo, selectedCity, environmentContext]);
  ...
  const aqi = useMemo(() => {
    ...
    return buildScenarioAqiData(baseAqi, scenario, currentAqi, ecologicalShiftPct, selectedCity, environmentContext);
  }, [baseAqi, scenario, currentAqi, ecologicalShiftPct, selectedCity, environmentContext]);
  ...
  const scenarioSignature = useMemo(
    () => [
      scenario.pm25ChangePct,
      scenario.no2ChangePct,
      scenario.coChangePct,
      scenario.deforestationPressurePct,
      scenario.biodiversityResiliencePct,
      scenario.waterQualityResiliencePct,
    ].join('-'),
    [scenario],
  );
          ...
          <div className="predictions-grid">
            ...
            <div className="pred-card">
              ...
              {co2 && (
                <div className="chart-container">
                  <Line
                    key={`co-${selectedCityKey}-${year}-${scenarioSignature}`}
                    data={{
                      labels: co2.labels,
                      datasets: [
                        {
            ...
            <div className="pred-card">
              ...
              {aqi && (
                <div className="chart-container">
                  <Bar
                    key={`aqi-${selectedCityKey}-${year}-${scenarioSignature}`}
                    data={{
                      labels: aqi.labels,
                      datasets: [
                        {
